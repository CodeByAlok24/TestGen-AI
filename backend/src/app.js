...
import { buildCacheKey, generateOtpCode, signToken } from './utils.js'
...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', redis: isRedisAvailable() ? 'connected' : 'disabled' })
})
...
app.post('/api/auth/request-otp/', async (req, res) => {
  ...
  if (isRedisAvailable()) {
    await setRedisJson(otpKey, otpPayload, config.otpTtlSeconds)
  } else {
  ...
  res.json({
    message: isRedisAvailable()
      ...
      : 'OTP generated with local fallback store.',
    otp,
...
app.post('/api/auth/verify-otp/', async (req, res) => {
  ...
  const otpKey = `auth:otp:${user.id}`
  const otpRecord = isRedisAvailable()
    ? await getRedisJson(otpKey)
    : await getOtpRecord(otpKey)
  ...
  res.json({
    ...
    email: user.email,
    message: isRedisAvailable()
      ? 'OTP verified and login session stored in Redis.'
      : 'OTP verified and login session stored with local fallback.',
  })
})
...
async function persistAuthSession(decodedToken, token) {
  ...
  if (isRedisAvailable()) {
    await setRedisJson(sessionKey, payload, config.authSessionTtlSeconds)
  } else {
